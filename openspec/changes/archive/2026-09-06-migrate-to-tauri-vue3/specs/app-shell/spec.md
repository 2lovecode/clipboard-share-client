...
The configuration UI SHALL allow the user to host a listening session (port + shared passphrase), join a peer (address/port + passphrase), view a generated passphrase when hosting, and return to the history surface. Connection outcomes MUST surface via the shared connection status contract.

#### Scenario: Host from configuration
- **WHEN** the user submits host settings with a valid port and passphrase
- **THEN** the system begins listening for inbound peers and updates connection status accordingly

#### Scenario: Join from configuration
- **WHEN** the user submits a peer address, port, and matching passphrase
